...
export default function CorteCaja() {
      ...
      try {
        ...
        if (turno === "manana") {
          ...
          fin = corteManana?.fechaCorte?.toDate
            ...
            : dayjs().hour(15).minute(0).second(0).toDate();
        } else {
        ...
        // Sumar efectivo y dólares recibidos en ventas
        const sumaMXN = datos
          .filter((v) => v.estado !== "cancelada")
          .reduce((acc, v) => acc + (Number(v.efectivoMXN) || 0), 0);

        const sumaUSD = datos
          .filter((v) => v.estado !== "cancelada")
          .reduce((acc, v) => acc + (Number(v.efectivoUSD) || 0), 0);

        setEfectivoEnCaja(sumaMXN.toFixed(2));
        setDolaresEnCaja(sumaUSD.toFixed(2));
      } catch (err) {
  ...
  const inicioActual =
    Number(turno === "manana" ? inicioManana : inicioTarde) || 0;
  const efectivoEnCajaNum = Number(efectivoEnCaja) || 0;
  const dolaresEnCajaNum = Number(dolaresEnCaja) || 0;
  const efectivoConInicio = +(efectivoEnCajaNum + inicioActual).toFixed(2);
  const totalEnCajaMXN = +(
    efectivoConInicio +
    dolaresEnCajaNum * (tipoCambio || 0)
  ).toFixed(2);
  ...
  const abrirConfirmCorte = async () => {
    ...
    ventasDia.forEach((v) => {
      ...
      // Normalizar campos (soporta variantes/legacy fields)
      const efectivoMXN =
        Number(v.efectivoMXN ?? v.efectivo ?? v.entregado?.mxn ?? 0) || 0;
      const efectivoUSD =
        Number(v.efectivoUSD ?? v.usd ?? v.entregado?.usd ?? 0) || 0;
      const tarjeta = Number(v.tarjeta ?? v.entregado?.tarjeta ?? 0) || 0;
      const transferencia =
        Number(v.transferencia ?? v.entregado?.transferencia ?? 0) || 0;

      // Sumar efectivo (MXN + USD convertido)
      const efectivoConvertido = efectivoMXN + efectivoUSD * (tipoCambio || 0);
      if (efectivoConvertido > 0) {
        desglose.efectivo += efectivoConvertido;
        conteo.efectivo += 1;
      }

      // Sumar tarjeta y transferencia por separado (sin restarles cambio)
      if (tarjeta > 0) {
        desglose.tarjeta += tarjeta;
        conteo.tarjeta += 1;
      }
      if (transferencia > 0) {
        desglose.transferencia += transferencia;
        conteo.transferencia += 1;
      }

      // Total (suma de los montos aplicados a caja)
      desglose.total += efectivoConvertido + tarjeta + transferencia;
    });
    // propinas
  ...
  return (
    <section className="bg-white rounded shadow p-4 mt-10 sm:mt-0">
        ...
        <div className="p-6 border rounded bg-yellow-50 text-center">
          ...
          <div className="flex justify-center gap-2">
            <button
              ...
              className="px-4 py-2 bg-[var(--color-principal)] text-white rounded"
            >
              ...
              <div className="mb-4">
                ...
                <div className="mt-2 text-sm text-gray-600">
                  ...
                  <span className="font-bold">
                    {ventasDia.filter((v) => v.estado !== "cancelada").length}
                  </span>
                </div>
              </div>
                ...
                <>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="mt-2 text-sm text-gray-600">
                      ...
                      <span className="font-bold">
                        {
                          ventasDia.filter((v) => v.estado !== "cancelada")
                            .length
                        }
                      </span>
                    </div>
      ...
      {rol === "admin" && (
        <div className="mt-6">
          <h4 className="font-semibold text-[var(--color-principal)] flex items-center gap-2 mb-2">
        ...
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-black bg-opacity-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-lg p-6">
            ...
            <div className="mb-4 text-right font-bold">
              Total ventas: ${desglosePagos.total.toFixed(2)}
            </div>
            <div className="flex flex-wrap sm:flex-nowrap border-y-2 border-dotted border-yellow-400 items-center justify-between my-2">
              ...
              <div className="mb-4 text-right font-bold w-full">
                Total en caja: ${totalEnCajaMXN.toFixed(2)} MXN
              </div>
            </div>
            <div className="flex justify-end gap-2">
              <button
