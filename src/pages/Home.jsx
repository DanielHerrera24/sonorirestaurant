...
const Home = () => {
  ...
  const cargarCatalogoRecompensas = async () => {
    const q = query(
      collection(db, "catalogoRecompensas"),
      where("activo", "==", true)
    );
    ...
    setCatalogoRecompensas(recompensas);
  };

  useEffect(() => {
    cargarCatalogoRecompensas();
  }, []);
      ...
      {!loading && !loadError && (
        <>
          ...
          >
            {datos && (
              <div className="flex flex-col items-center justify-center gap-2">
                ...
                {/* Información de la tarjeta de lealtad */}
                {!rol ||
                  (rol === "cliente" && (
                ...
                {rol === "admin" && (
                  <>
                    <PanelAdmin />
                    <RegistrarVisita user={user} clienteUid={clienteUid} />
                  </>
                )}
                {rol === "caja" && (
                  <>
          ...
          {/* {rol === "cliente" && (
            ...
            >
              {catalogoRecompensas.length > 0 && (
                <div className="w-full mb-2">
                  <h3 className="text-4xl text-[var(--color-negro)] mb-2 text-center">
                    Recompensas que puedes ganar
                  </h3>
                  <ul className="space-y-2">
                    {catalogoRecompensas.map((r, i) => (
                      <li
                        key={r.nombre + i}
                        className="bg-[var(--color-blanco)] border border-orange-200 rounded p-2 flex flex-col items-start"
                      >
                        <span className="text-3xl text-[var(--color-negro)]">
                          🎁 {r.nombre}
                        </span>
                        {r.descripcion && (
